...
try:
    ...
    # Argos Translate (and its dependencies like spaCy/pydantic) may be
    # incompatible with the current Python version. Disable translation
    # functionality but allow the rest of the app to run.
...
try:
    if translation_available:
        ...
        if package_to_install is not None:
            ...
            print(f"No Argos Translate package found for {from_code} -> {to_code}. Continuing without auto-install.")
    else:
        print('Argos Translate unavailable; translation disabled.')
except Exception:
    # Don't crash the whole app if network or package lookup fails; print traceback and continue.
    traceback.print_exc()
...
def main():
    try:
        ...
        # Load app icon safely using absolute path
        ...
        image_path = os.path.join(os.path.dirname(__file__), "images", "signstpw.png")
        if os.path.exists(image_path):
            ...
            label = Label(root, text="Image not found", bg="#1C1C1C", fg="white")
                    ...
                    if  count>=10 and expression[-1] !=  sign:
                        ...
                        if sign == "fullstop":
                            ...
                            if translation_available:
                                ...
                                translatedText = '<translation-disabled>'
        ...
        # Print full traceback to stderr so that issues are visible during development.
        traceback.print_exc()
        sys.exit(1)
